...
def get_homefeed_all_channel(request: GetHomefeedAllChannelRequest):
    """获取主页的所有频道"""
    return handle_api_call(xhs_apis.get_homefeed_all_channel, request.cookies_str,None)
...
def get_homefeed_recommend(request: GetHomefeedRecommendRequest):
    """获取主页推荐的笔记"""
    return handle_api_call(xhs_apis.get_homefeed_recommend, request.category, request.cursor_score, request.refresh_type, request.note_index, request.cookies_str,None)
...
def get_homefeed_recommend_by_num(request: GetHomefeedRecommendByNumRequest):
    """根据数量获取主页推荐的笔记"""
    return handle_api_call(xhs_apis.get_homefeed_recommend_by_num, request.category, request.require_num, request.cookies_str,None)
...
def get_user_info(request: GetUserInfoRequest):
    """获取用户的信息"""
    return handle_api_call(xhs_apis.get_user_info, request.user_id, request.cookies_str,None)
...
def get_user_self_info(request: GetUserSelfInfoRequest):
    """获取用户自己的信息1"""
    return handle_api_call(xhs_apis.get_user_self_info, request.cookies_str,None)

@app.post("/api/get_user_self_info2")
def get_user_self_info2(request: GetUserSelfInfoRequest):
    """获取用户自己的信息2"""
    return handle_api_call(xhs_apis.get_user_self_info2, request.cookies_str,None)
...
def get_user_note_info(request: GetUserNoteInfoRequest):
    """获取用户指定位置的笔记"""
    return handle_api_call(xhs_apis.get_user_note_info, request.user_id, request.cursor, request.cookies_str, request.xsec_token, request.xsec_source,None)
...
def get_user_all_notes(request: GetUserAllNotesRequest):
    """获取用户所有笔记"""
    return handle_api_call(xhs_apis.get_user_all_notes, request.user_url, request.cookies_str,None)
...
def get_user_like_note_info(request: GetUserLikeNoteInfoRequest):
    """获取用户指定位置喜欢的笔记"""
    return handle_api_call(xhs_apis.get_user_like_note_info, request.user_id, request.cursor, request.cookies_str, request.xsec_token, request.xsec_source,None)
...
def get_user_all_like_note_info(request: GetUserAllLikeNoteInfoRequest):
    """获取用户所有喜欢笔记"""
    return handle_api_call(xhs_apis.get_user_all_like_note_info, request.user_url, request.cookies_str,None)
...
def get_user_collect_note_info(request: GetUserCollectNoteInfoRequest):
    """获取用户指定位置收藏的笔记"""
    return handle_api_call(xhs_apis.get_user_collect_note_info, request.user_id, request.cursor, request.cookies_str, request.xsec_token, request.xsec_source,None)
...
def get_user_all_collect_note_info(request: GetUserAllCollectNoteInfoRequest):
    """获取用户所有收藏笔记"""
    return handle_api_call(xhs_apis.get_user_all_collect_note_info, request.user_url, request.cookies_str,None)
...
def get_note_info(request: GetNoteInfoRequest):
    """获取笔记的详细"""
    return handle_api_call(xhs_apis.get_note_info, request.url, request.cookies_str,None)
...
def get_search_keyword(request: GetSearchKeywordRequest):
    """获取搜索关键词"""
    return handle_api_call(xhs_apis.get_search_keyword, request.word, request.cookies_str,None)
...
def search_note(request: SearchNoteRequest):
    """获取搜索笔记的结果"""
    return handle_api_call(xhs_apis.search_note, request.query, request.cookies_str, request.page, request.sort, request.note_type,None)
...
def search_some_note(request: SearchSomeNoteRequest):
    ...
    return handle_api_call(xhs_apis.search_some_note, request.query, request.require_num, request.cookies_str, request.sort, request.note_type,None)
...
def search_user(request: SearchUserRequest):
    """获取搜索用户的结果"""
    return handle_api_call(xhs_apis.search_user, request.query, request.cookies_str, request.page,None)
...
def save_to_notion(request: SaveToNotionRequest):
    return handle_api_call(notion_api.save_xiaohongshu_note_to_notion, request.notion_token, request.database_id,request.cookies_arr,request.note_url,request.remarks,
                           request.custom_tags,request.video_transfer,request.video_transfer_api_key,None)
